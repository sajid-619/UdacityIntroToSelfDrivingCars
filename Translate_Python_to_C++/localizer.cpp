	...
	float p_miss)
{
	...
	int height = grid.size();
  int width = grid[0].size();
	vector< vector <float> > newGrid (height, vector <float> (width, 0.0)); // new_beliefs

	for(int i = 0; i < height;  i++){
			for(int j = 0; j < width; j++){
					int t = (color == grid[i][j]);
					newGrid[i][j] = (newGrid[i][j] * (t * p_hit + (1-t) * p_miss));
				}
			}
	...
	float blurring)
{
	...
	for (int i=0; i < height; i++){
			for(int j=0; j < width; j++){
					...
					new_i = new_i >= 0 ? new_i: height+new_i; // adjust for negative indices
					new_j = new_j >= 0 ? new_j: height+new_j; // adjust for negative indices
